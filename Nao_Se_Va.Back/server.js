...
app.post('/send-email', async (req, res) => {
  ...
  try {
    const htmlContent = `
      <div align="center" style="font-family: Arial, Helvetica, sans-serif; background-color: #f4f4f4; padding: 20px;">
        <div style="max-width: 600px; background: white; padding: 30px; border-radius: 10px;">
          ...
          <p style="font-size: 16px; font-weight: bold; color: #555;">Enviado por: 
            <span style="font-size: 15px; color: #333;">${email}</span>
          </p>
          <p style="font-size: 16px; color: #555;">${text}</p>
          <hr style="margin: 30px 0;">
          <p style="font-size: 12px; color: #888; text-align: center;">
            Se você não reconhece o remetente deste e-mail ou acredita que foi enviado por engano,
          </p>
          <p style="font-size: 12px; color: #888; text-align: center;">
...
app.get('/aluno', async (req, res) => {
  ...
  const { idAluno } = req.query; // agora vem da query
  ...
  try {
    const response = await axios.get(
      ...
      {
        params: { idAluno }, // passa o idAluno como query string
        httpsAgent: agent,
        headers: {
          Accept: 'application/json',
          Authorization: `Bearer ${token}`, // aqui o token é passado
